...
function normalizeCreature(rawItem: unknown): Creature | null {
  ...
  return {
    ...
    trait: {
      ...
      description: String(
        (item.trait as Record<string, unknown>)?.description || ''
      ).trim(),
    },
    images: {
      default: String(
        (item.images as Record<string, unknown>)?.default || ''
      ).trim(),
      base: String(
        (item.images as Record<string, unknown>)?.base || ''
      ).trim(),
      forms:
        ((item.images as Record<string, unknown>)?.forms as Record<
          string,
          string
        >) || {},
      bossForms:
        ((item.images as Record<string, unknown>)?.bossForms as Record<
          string,
          string
        >) || {},
      bossFormVariants:
        ((item.images as Record<string, unknown>)?.bossFormVariants as Record<
          string,
          Record<string, string>
        >) || {},
    },
    raceStats: {
      default: normalizeStats(
        (item.raceStats as Record<string, unknown>)?.default
      ),
      forms:
        ((item.raceStats as Record<string, unknown>)?.forms as Record<
          string,
          Stats
        >) || {},
      bossForms:
        ((item.raceStats as Record<string, unknown>)?.bossForms as Record<
          string,
          Stats
        >) || {},
      bossFormVariants:
        ((item.raceStats as Record<string, unknown>)?.bossFormVariants as Record<
          string,
          Record<string, Stats>
        >) || {},
    },
...
function normalizeLearnset(rawItem: unknown): LearnsetEntry | null {
  ...
  return {
    ...
    bossFormVariants:
      (item.bossFormVariants as Record<string, Record<string, string[]>>) || {},
  };
}
...
// ============== 客户端组件 ==============
export default function CreatureDetailClient({ creatureId }: { creatureId: string }) {
    ...
    async function loadCreatureDetail() {
      ...
      try {
        ...
        const matchedCreature = creatures.find(
          (item: { id?: string }) => String(item?.id || '').trim() === creatureId
        );
        if (!matchedCreature) {
        ...
        if (fileName) {
          result.push({
            ...
            stats: normalizeStats(
              raceStats.bossFormVariants?.[bossLabel]?.[variantLabel]
            ),
          });
        }
      });
  ...
  const currentForm = useMemo(() => {
    ...
    return (
      formOptions.find((item) => item.key === selectedFormKey) || formOptions[0]
    );
  ...
  function getCurrentSkillIds(): string[] {
    ...
    if (parsed.type === 'boss-variant') {
      const variantSkills =
        learnsetEntry.bossFormVariants?.[parsed.bossLabel]?.[parsed.variantLabel];
